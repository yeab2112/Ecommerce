    ...
    <div className="product-detail-container p-6 flex flex-col gap-8 w-full max-w-6xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
        ...
        <div className="details-section flex flex-col gap-4 order-2 lg:order-3 xl:order-2">
          ...
          {product.bestSeller && (
            <span className="bg-yellow-100 text-yellow-800 text-xs font-medium px-2.5 py-0.5 rounded w-fit">
              Bestseller
            </span>
          )}
        ...
        <div className="product-info order-3 lg:order-2 xl:order-3 bg-gray-50 p-6 rounded-lg">
          ...
          <ul className="space-y-3">
            ...
            <li className="flex items-start">
              ...
              <span>
                <strong>Available Colors:</strong> {product.colors?.map(c => c.name).join(', ')}
              </span>
            </li>
            <li className="flex items-start">
              <svg className="w-5 h-5 text-blue-500 mr-2 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <span><strong>Available Sizes:</strong> {product.sizes?.join(', ')}</span>
            </li>
          </ul>
        </div>
      </div>
